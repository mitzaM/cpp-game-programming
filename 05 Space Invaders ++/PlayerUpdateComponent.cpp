...
void PlayerUpdateComponent::update(float fps)
{
    ...
    m_RCC->setOrMoveCollider(
        m_TC->getLocation().x, m_TC->getLocation().y, size.x, size.y
    );

    if (m_TC->getLocation().x > WorldState::WORLD_WIDTH - size.x) {
        m_TC->getLocation().x = WorldState::WORLD_WIDTH - size.x;
